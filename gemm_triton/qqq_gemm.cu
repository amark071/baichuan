...
//这个 Vec 结构体提供了一个轻量级的、固定大小的向量抽象,可用于 GPU 加速的计算中。固定大小允许编译器优化内存布局和访问模式,这对 GPU 性能很重要。
...
//这个函数利用CUDA汇编语言的能力执行异步内存拷贝操作，并且通过预测寄存器来控制是否执行拷贝。
//它主要用于高性能计算中，需要精确控制内存拷贝行为的场景。
...
//这是一条 CUDA 汇编指令,用于等待一组异步内存拷贝操作完成。
