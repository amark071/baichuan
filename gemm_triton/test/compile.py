...
if __name__ == "__main__":
    ...
    meta_sig = f"warps{int(args.num_warps)}xstages{int(args.num_stages)}"
    a=signature + [meta_sig]
    print(a)
    sig_hash = hash_signature(a)
    print(sig_hash)
    def constexpr(s):
        try:
            ret = int(s)
    ...
    doc_string += [f"num_warps={int(args.num_warps)}", f"num_stages={int(args.num_stages)}"]
    ...
    opts = {"num_warps": int(args.num_warps), "num_stages": int(args.num_stages)}
