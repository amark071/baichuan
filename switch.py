...
def matmul(a, b, activation=""):
    ...
    M, K = a.shape
    K, N = b.shape
    ldtype = str(a.dtype).split('.')[-1]
    rdtype = str(b.dtype).split('.')[-1]
    
    if(ldtype == "float16"):
        lt=1
    elif(ldtype == "float32"):
        lt=2
    elif(ldtype == "bfloat16"):
        lt=3
    elif(ldtype == "int8"):
        lt=4
    elif(ldtype == "int4"):
    ...
    dtype_c = getattr(torch, c_dtype)
    c = torch.empty((M, N), device=a.device, dtype=dtype_c)
...
with open('matrix_config.json', 'r') as file:
    data = json.load(file)
m_num=[]
n_num=[]
k_num=[]
...
for item in data:
    ...
    n_num.append(item["N"])
    k_num.append(item["K"])
    ldtype.append(item["ldtype"])
    rdtype.append(item["rdtype"])
    
numbers = []
for i in range(0,len(m_num)):
    m=m_num[i]
    n=n_num[i]
    k=k_num[i]
    dtype_left = getattr(torch, ldtype[i])
    dtype_right = getattr(torch, rdtype[i])
    if ldtype[i] == "int4" or "int8":
        a1 = torch.randint(low=-128, high=127, size=(m,k), dtype=dtype_left, device='cuda')
    else:
        a1 = torch.randn((m,k), device='cuda', dtype=dtype_left)

    if rdtype[i] == "int4" or "int8":
        b1 = torch.randint(low=-128, high=127, size=(k,n), dtype=dtype_right, device='cuda')
    else:
        b1 = torch.randn((k,n), device='cuda', dtype=dtype_right)
    ...
    numbers.append(n)
    numbers.append(k)
    numbers.append(ldtype[i])
    numbers.append(rdtype[i])
...
grouped_data = []
for i in range(0, len(numbers),11):
        group = {label: numbers[i+j] for j, label in enumerate(labels)}
